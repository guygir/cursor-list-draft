...
- Credibility and demand meters sit on the draft rail from the first pick and recompute after every pick. Hover (desktop) or tap (phone) opens a one-line explainer: tree vs hill. Bars grow and numbers count up (Fermi.gg motion). Demand fill is punch coral `#ff5e3a`; credibility stays mandate ochre. Election night uses cream/coral score cards: seat count-up first, then mandate bars, then meters. Reduced motion snaps.
