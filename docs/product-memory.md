...
- Shared pool is 2026 published top slots (10 where the slate published 10; Ra'am and Blue and White only published 6 named slots). Snake draft vs 1–3 CPUs now fills 6 names each.
...
- Wikipedia thumbnails on the tree (HTML faces) and member rows when a page photo exists; otherwise an initial. Identity art, not an endorsement.
- Opponent picks this snake pass show as side notices on the draft graph.
- CPU maximises projected election-night votes (cohesion + split hill + hub), not a shallow draw×compat heuristic. Hard mode looks one partner ahead on a new slate. Rare near-tie noise.
