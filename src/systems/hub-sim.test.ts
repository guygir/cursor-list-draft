...
import { playAllNoisy, playDraft, runHubCell } from "./hub-sim";
...
describe("noisy hub grid", () => {
  ...
  it("can keep the player greedy while CPUs stay noisy", () => {
    const finished = playDraft(createDraft(2, 4, "open", "liberman"), "greedy");
    const player = finished.lists.find((list) => list.isPlayer);
    expect(player?.picks[0]).toBe("liberman");
    expect(finished.lists.every((list) => list.picks.length === 10)).toBe(true);
    const seats = [1, 2, 3, 4, 5, 6, 7, 8].map((seed) => ({
      greedy: runHubCell("bennett", 2, "three", seed, "greedy").playerSeats,
      noisy: runHubCell("bennett", 2, "three", seed, "noisy").playerSeats,
    }));
    expect(seats.some((row) => row.greedy !== row.noisy)).toBe(true);
  });
});
